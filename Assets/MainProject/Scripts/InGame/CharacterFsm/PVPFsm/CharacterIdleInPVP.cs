...
{
    public class CharacterIdleInPVP : CharacterIdle, IStateCallback
    {
        [Header("적 감지")]
        ...
        protected override void onUpdate()
        {
            ...
            {
                var enemycharacter = hit.collider.gameObject.GetComponent<Character>();
                if(enemycharacter._health.CurrentHealth<=0)
                {
                ...
                else
                {
                    ...
                    {
                        if (enemycharacter.playertype == (_character as CharacterInPVP).enemytype)
                        {
